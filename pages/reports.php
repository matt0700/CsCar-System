...
<div class="w3-main ">
    ...
    $mrotSql = "SELECT m.mrot_id, v.make_series_type, m.mileage_trip, m.fuel_trip, m.issue, m.submitted 
                FROM mrot m
                LEFT JOIN vehicle_data v ON m.plate_no = v.plate_no
    ...
    echo "<div class='overflow-x-auto'>"; // Added container for overflow
    echo "<table class='min-w-full bg-white'>";
    echo "<thead class='bg-gray-800 text-white'>";
    echo "<tr>";
    echo "<th class='w-1/6 px-4 py-2 '><a href='?sortBy=mrot_id&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>MROT ID</a></th>";
    echo "<th class='w-1/6 px-4 py-2 '><a href='?sortBy=make_series_type&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>Vehicle Name</a></th>";
    echo "<th class='w-1/6 px-4 py-2'><a href='?sortBy=mileage_trip&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>Mileage Trip</a></th>";
    echo "<th class='w-1/6 px-4 py-2'><a href='?sortBy=fuel_trip&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>Fuel Trip</a></th>";
    echo "<th class='w-1/6 px-4 py-2'><a href='?sortBy=issue&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>Issue</a></th>";
    echo "<th class='w-1/6 px-4 py-2'><a href='?sortBy=submitted&sortOrder=" . ($sortOrder == 'asc' ? 'desc' : 'asc') . "' class='text-white no-underline'>Submitted</a></th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    ...
    if ($mrotResult->num_rows > 0) {
        while ($row = $mrotResult->fetch_assoc()) {
            ...
            echo "<td class='border px-4 py-2'>" . htmlspecialchars($row['make_series_type']) . "</td>";
